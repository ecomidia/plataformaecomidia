  ...
  <q-page class="column">
    ...
    <div class="temas">
      ...
      <div class="" style="margin-top: 40px;">
        <masonry-wall v-if="tipo.selected == 'arvore'" class="q-pt-xl" :items="statusStore.hashtags" :ssr-columns="1"
          :column-width="300" :gap="16">
          <template #default="{ item }">
            <div class="twittertree flex flex-center"
              :style="`height: ${relativeHeight(statusStore.hashtags, item)}px; width: 100%;`">
              <span class="treetext">
                #{{ item.hashtags }}
              </span>
            </div>
        ...
        <table v-if="tipo.selected == 'tabela'" style="width:100%">
          ...
          <tr v-for="item in statusStore.hashtags" :key="item.hashtags">
            <td class="q-py-xs q-px-md">#{{ item.hashtags }}</td>
        ...
        <div v-if="tipo.selected == 'barras'">
          ...
          <div class="row items-end">
            <div class="q-px-sm col" v-for="item in statusStore.hashtags.slice(0, 8)" :key="item.hashtags">
              <div class="twittertree flex flex-center"
                :style="`height: ${relativeHeight(statusStore.hashtags, item)}px; width: 100%;`">
                <span class="treetext">
          ...
          <q-separator class="q-mt-md" style="background: #6B7280;" size="2px"></q-separator>
          <div class="row">
            <div class="q-px-md q-mt-sm col" v-for="item in statusStore.hashtags.slice(0, 8)" :key="item.hashtags">
              <span class="barraheader">#{{ item.hashtags }}</span>
...
async function getData() {
  ...
  url = proxyurl + encodeURIComponent(`${baseurl}/hashtags${filters}&n=12`)
  await axios
    .get(url)
    .then(response => {
